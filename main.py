...
def add_order():
    ...
    cur.execute("INSERT INTO orders (customer_name, order_details, status) VALUES (?, ?, 'Новый')",
                (cuctomer_name_entry.get(), order_details_entry.get()))
    conn.commit()
    conn.close()

    cuctomer_name_entry.delete(0, tk.END)
...
tk.Label(app, text="имя  клиеента").pack()

cuctomer_name_entry = tk.Entry(app)
cuctomer_name_entry.pack()
